...
export default function EditQuestModal({ templateId, token, onSave, onClose }) {
  ...
  // Fetch template and wait for Groq to populate it
  useEffect(() => {
    const fetchTemplate = async () => {
      try {
        // Wait a bit for Groq background task to complete
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div
        className="w-full max-w-2xl p-6 md:p-8 rounded-lg shadow-xl max-h-[90vh] overflow-y-auto"
