...
from app.models.quest import QuestCreate, QuestTemplateCreate
...
def seed_database():
    ...
    with Session(engine) as session:
        ...
        for i, (title, description, xp, gold, recurrence) in enumerate(quest_data):
            ...
            print(
                f"  ✓ Created template: {template.title} [{template.quest_type}] ({template.recurrence}) ({template.xp_reward} XP)"
            )
        ...
        for name, description, cost in reward_data:
            reward_in = RewardCreate(name=name, description=description, cost=cost)
